...
import { DOC_PICK_PROVIDER } from './doc-pick.provider';
import { DocPickTestService } from './doc-pick-test.service'
...
import { PdAvailableByStorageReturnType } from '../pd-storage/pd-storage.interface';
...
describe('DOC_PICK', () => {
    ...
    let docPickTestService: DocPickTestService
    ...
    let _docPick: DocPick;
    let _pdWhMap: { [key: string]: PdWh[]; };

    let docId = '74n6qcrmF4tKTq8kWkhz6Y';
    ...
    beforeAll(async () => {
        ...
        docPickTestService = app.get<DocPickTestService>(DocPickTestService);
        ...
        _biz = {
            bizId: docPickTestService.service.generateId(),
            bizNo: 'BIZ-01',
            bizName: 'BIZ-NAME-01'
        }

        _branch = {
            branchId: docPickTestService.service.generateId(),
        }

        _whs = [
            {
                whId: docPickTestService.service.generateId(),
                whNo: 'WH-01',
                whName: 'WH-NAME-01',
                bizId: _biz.bizId,
        ...
        _whs.forEach(wh => {
            ['01', '02', '03'].forEach(sufix => {
                // Zone
                const whZone: WhZone = {
                    whZoneId: docPickTestService.service.generateId(),
                    whZoneNo: sufix,
                ...
                const whAisle: WhAisle = {
                    whAisleId: docPickTestService.service.generateId(),
                ...
                // Rack
                const whRack: WhRack = {
                    whRackId: docPickTestService.service.generateId(),
                ...
                const whShelf: WhShelf = {
                    whShelfId: docPickTestService.service.generateId(),
                ...
                ['1', '2', '3'].forEach(l => {
                    ...
                    const whStorage: WhStorage = {
                        whStorageId: docPickTestService.service.generateId(),
                        whId: wh.whId,
    ...
    describe('DOC_PICK Discrete Picking', () => {
        ...
        beforeAll(async () => {
            ...
            _pds = _whStorages.map( ws => {
                const pd: Pd = {
                    pdId: docPickTestService.service.generateId(),
                ...
                const pdStorage: PdStorage = {
                    pdStorageId: docPickTestService.service.generateId(),
                    pdId: pd.pdId,
                    whId: ws.whId,
            ...
            _pdBranches = _pds.map(pd => {
                return {
                    pdBranchId: docPickTestService.service.generateId(),
                    pdId: pd.pdId,
                    ohQty: 200,
                    allocatedQty: 0,
                    bizId: _biz.bizId,
            ...
            _whs.forEach( wh => {
                _pdStorages.filter( s => s.whId === wh.whId).forEach( pdStorage => {
                    const pdWh: PdWh = {
                        pdWhId: docPickTestService.service.generateId(),
                        pdId: pdStorage.pdId,
                        whId: wh.whId,
                        ohQty: 200,
                        allocatedQty: 0,
        ...
        describe('Pick in the same warehouse', () => {
            ...
            it('#1.0 Create Pick Order', async () => {
                ...
                _docPick = await docPickTestService.add({ bid, brid, data: _docPick })

                expect(true).toBe(true)
            })
            it('#1.1 Branch exception', async () => {
                const docPick = await docPickTestService.getById(bid, { id: docId })
                ...
                const after = await docPickTestService.checkAvailabity({ bid, brid, docPick })
            ...
            it('#1.1 Same Wh exception', async () => {
                ...
                const pdWhs = await docPickTestService.pdWhService.get({ bid, brid, filter: {  _and: [ { whId: docPick.whId }, { _in: { pdId: pdIds } } ] } } );
            ...
            it('#1.1 Zone exception', async () => {
                const docPick = await docPickTestService.getById(bid, { id: docId });
                const pickPds = docPick.pd;
                const pd0: any = await docPickTestService.pdStorageService.pdAvailableByZone({ bid, brid, whZoneId: pickPds[0].fromZoneId, pdId: pickPds[0].pdId})
                const pd1: any = await docPickTestService.pdStorageService.pdAvailableByZone({ bid, brid, whZoneId: pickPds[1].fromZoneId, pdId: pickPds[1].pdId})
                const pd2: any = await docPickTestService.pdStorageService.pdAvailableByZone({ bid, brid, whZoneId: pickPds[2].fromZoneId, pdId: pickPds[2].pdId})
            ...
            it('#1.1 Aisle exception', async () => {
                ...
                const pd0: any = await docPickTestService.pdStorageService.pdAvailableByAisle({ bid, brid, whAisleId: pickPds[0].fromAisleId, pdId: pickPds[0].pdId})
                const pd1: any = await docPickTestService.pdStorageService.pdAvailableByAisle({ bid, brid, whAisleId: pickPds[1].fromAisleId, pdId: pickPds[1].pdId})
                const pd2: any = await docPickTestService.pdStorageService.pdAvailableByAisle({ bid, brid, whAisleId: pickPds[2].fromAisleId, pdId: pickPds[2].pdId})
            ...
            it('#1.1 Rack exception', async () => {
                ...
                const pd0: any = await docPickTestService.pdStorageService.pdAvailableByRack({ bid, brid, whRackId: pickPds[0].fromRackId, pdId: pickPds[0].pdId})
                const pd1: any = await docPickTestService.pdStorageService.pdAvailableByRack({ bid, brid, whRackId: pickPds[1].fromRackId, pdId: pickPds[1].pdId})
                const pd2: any = await docPickTestService.pdStorageService.pdAvailableByRack({ bid, brid, whRackId: pickPds[2].fromRackId, pdId: pickPds[2].pdId})
            ...
            it('#1.1 Shelf exception', async () => {
                ...
                const pd0: any = await docPickTestService.pdStorageService.pdAvailableByShelf({ bid, brid, whShelfId: pickPds[0].fromShelfId, pdId: pickPds[0].pdId})
                const pd1: any = await docPickTestService.pdStorageService.pdAvailableByShelf({ bid, brid, whShelfId: pickPds[1].fromShelfId, pdId: pickPds[1].pdId})
                const pd2: any = await docPickTestService.pdStorageService.pdAvailableByShelf({ bid, brid, whShelfId: pickPds[2].fromShelfId, pdId: pickPds[2].pdId})
            ...
            it('#1.1 Storage exception', async () => {
                ...
                const pd0: any = await docPickTestService.pdStorageService.pdAvailableByStorage({ bid, brid, whStorageId: pickPds[0].fromStorageId, pdId: pickPds[0].pdId})
                const pd1: any = await docPickTestService.pdStorageService.pdAvailableByStorage({ bid, brid, whStorageId: pickPds[1].fromStorageId, pdId: pickPds[1].pdId})
                const pd2: any = await docPickTestService.pdStorageService.pdAvailableByStorage({ bid, brid, whStorageId: pickPds[2].fromStorageId, pdId: pickPds[2].pdId})
                ...
                const pickOrd: PickOrdInfo[] = [
                    {
                        ...
                        items: [
                            {
                                itemId: docPickTestService.service.generateId(),
        ...
        it('#1.1 Wh exception', async () => {
            ...
            const pdWhs = await docPickTestService.pdWhService.get({ bid, brid, filter: {  _and: [ { _in: { whId: whIds } }, { _in: { pdId: pdIds } } ] } } );
            const pd0Wh = pdWhs.find( pd => pd.pdId === pickPds[0].pdId && pd.whId === pickPds[0].fromWhId)
            const pd1Wh = pdWhs.find( pd => pd.pdId === pickPds[1].pdId && pd.whId === pickPds[1].fromWhId)
            const pd2Wh = pdWhs.find( pd => pd.pdId === pickPds[2].pdId && pd.whId === pickPds[2].fromWhId)
            pickPds[0].qtyToPick = pd0Wh?.ohQty + 2;
